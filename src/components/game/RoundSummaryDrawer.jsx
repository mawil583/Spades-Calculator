        ...
        <SharedSection title="Penalties">
          <Stack style={team1Styles} alignItems="center" gap="var(--app-spacing-4)">
            {t1Derived.hasPenalties ? (
              <>
                {team1Stats.bagPenalty > 0 && <RoundSummaryField label="Bag Penalty" value={`-${team1Stats.bagPenalty}`} color="red.500" />}
                {team1Stats.setPenalty > 0 && <RoundSummaryField label="Set Penalty" value={`-${team1Stats.setPenalty}`} color="red.500" />}
                {team1Stats.nilPenalty > 0 && <RoundSummaryField label="Nil Penalty" value={`-${team1Stats.nilPenalty}`} color="red.500" />}
                {team1Stats.blindNilPenalty > 0 && <RoundSummaryField label="Blind Nil Penalty" value={`-${team1Stats.blindNilPenalty}`} color="red.500" />}
              </>
            ) : (
          ...
          <Stack style={team2Styles} alignItems="center" gap="var(--app-spacing-4)">
            {t2Derived.hasPenalties ? (
              <>
                {team2Stats.bagPenalty > 0 && <RoundSummaryField label="Bag Penalty" value={`-${team2Stats.bagPenalty}`} color="red.500" />}
                {team2Stats.setPenalty > 0 && <RoundSummaryField label="Set Penalty" value={`-${team2Stats.setPenalty}`} color="red.500" />}
                {team2Stats.nilPenalty > 0 && <RoundSummaryField label="Nil Penalty" value={`-${team2Stats.nilPenalty}`} color="red.500" />}
                {team2Stats.blindNilPenalty > 0 && <RoundSummaryField label="Blind Nil Penalty" value={`-${team2Stats.blindNilPenalty}`} color="red.500" />}
              </>
            ) : (
