...
const SharedSection = ({ title, children }: SharedSectionProps) => (
  <Stack gap="var(--app-spacing-2)" mb="var(--app-spacing-6)" alignItems="center" width="full">
...
function RoundSummaryDrawer({ team1Stats, team2Stats }: RoundSummaryDrawerProps) {
  ...
  return (
    <Stack mt="var(--app-spacing-4)" p="var(--app-spacing-4)" bg="bg">
      ...
      <SharedSection title="Net">
        <Stack style={team1Styles} alignItems="center" gap="var(--app-spacing-4)">
          <RoundSummaryField label="Points Gained" value={team1Stats.pointsGained} />
          <RoundSummaryField
            label="Points Lost"
        ...
        <Stack style={team2Styles} alignItems="center" gap="var(--app-spacing-4)">
          <RoundSummaryField label="Points Gained" value={team2Stats.pointsGained} />
          <RoundSummaryField
            label="Points Lost"
        ...
        <SharedSection title="Penalties">
          <Stack style={team1Styles} alignItems="center" gap="var(--app-spacing-4)">
            {t1Derived.hasPenalties ? (
              <>
                {(team1Stats.bagPenalty ?? 0) > 0 && <RoundSummaryField label="Bag Penalty" value={`-${team1Stats.bagPenalty}`} color="red.500" />}
                {(team1Stats.setPenalty ?? 0) > 0 && <RoundSummaryField label="Set Penalty" value={`-${team1Stats.setPenalty}`} color="red.500" />}
                {(team1Stats.nilPenalty ?? 0) > 0 && <RoundSummaryField label="Nil Penalty" value={`-${team1Stats.nilPenalty}`} color="red.500" />}
                {(team1Stats.blindNilPenalty ?? 0) > 0 && <RoundSummaryField label="Blind Nil Penalty" value={`-${team1Stats.blindNilPenalty}`} color="red.500" />}
              </>
            ) : (
          ...
          <Stack style={team2Styles} alignItems="center" gap="var(--app-spacing-4)">
            {t2Derived.hasPenalties ? (
              <>
                {(team2Stats.bagPenalty ?? 0) > 0 && <RoundSummaryField label="Bag Penalty" value={`-${team2Stats.bagPenalty}`} color="red.500" />}
                {(team2Stats.setPenalty ?? 0) > 0 && <RoundSummaryField label="Set Penalty" value={`-${team2Stats.setPenalty}`} color="red.500" />}
                {(team2Stats.nilPenalty ?? 0) > 0 && <RoundSummaryField label="Nil Penalty" value={`-${team2Stats.nilPenalty}`} color="red.500" />}
                {(team2Stats.blindNilPenalty ?? 0) > 0 && <RoundSummaryField label="Blind Nil Penalty" value={`-${team2Stats.blindNilPenalty}`} color="red.500" />}
              </>
            ) : (
      ...
      <SharedSection title="Bags">
        <Stack style={team1Styles} alignItems="center" gap="var(--app-spacing-4)">
          <RoundSummaryField label="Initial Bags" value={team1Stats.startBags} />
          <RoundSummaryField label="Bags Taken" value={t1Derived.bagsTaken} />
          <RoundSummaryField
            label={(team1Stats.bagPenalty ?? 0) > 0 ? 'Bag Result' : 'Total Bags'}
            value={(team1Stats.bagPenalty ?? 0) > 0 && team1Stats.endBags === 0 ? 'Reset to 0' : ((team1Stats.bagPenalty ?? 0) > 0 ? `Carryover: ${team1Stats.endBags}` : team1Stats.endBags)}
          />
        </Stack>
        <Stack style={team2Styles} alignItems="center" gap="var(--app-spacing-4)">
          <RoundSummaryField label="Initial Bags" value={team2Stats.startBags} />
          <RoundSummaryField label="Bags Taken" value={t2Derived.bagsTaken} />
          <RoundSummaryField
            label={(team2Stats.bagPenalty ?? 0) > 0 ? 'Bag Result' : 'Total Bags'}
            value={(team2Stats.bagPenalty ?? 0) > 0 && team2Stats.endBags === 0 ? 'Reset to 0' : ((team2Stats.bagPenalty ?? 0) > 0 ? `Carryover: ${team2Stats.endBags}` : team2Stats.endBags)}
          />
        </Stack>
      </SharedSection>
    </Stack>
