...
describe('isRoundComplete', () => {
  ...
  test('is true when every bid/actual is entered and actuals total 13', () => {
    expect(isRoundComplete(completeRound)).to.be.true;
  });

  test('is false while any team still has an empty actual', () => {
    const partial = {
      ...completeRound,
      team2BidsAndActuals: {
        ...completeRound.team2BidsAndActuals,
        p2Actual: '',
      },
    } as Round;
    expect(isRoundComplete(partial)).to.be.false;
  });

  test('is false when actuals do not total 13', () => {
    const wrongTotal = {
      ...completeRound,
      team2BidsAndActuals: {
        ...completeRound.team2BidsAndActuals,
    ...
    expect(isRoundComplete(wrongTotal)).to.be.false;
  });

  test('is false for null rounds or rounds missing a team', () => {
    expect(isRoundComplete(null)).to.be.false;
    expect(
