    ...
    >
      ...
      >
        <Flex align="center" mb={1} position="relative">
          <Text fontWeight="bold" fontSize="md" mr={!useTableRoundUI ? 1 : undefined}>
            {playerName}
          </Text>
...
const GameScoreDisplay = memo(({ teamName, score, bags, teamClassName }: GameScoreDisplayProps) => (
...
function TableRound({ roundHistory, isCurrent = false, roundIndex }: TableRoundProps) {
  ...
  const handleOpenModal = useCallback(({ fieldToUpdate, type, playerName, onCustomUpdate }: ModalOpenArgs) => {
    setModalState({
      isOpen: true,
      fieldToUpdate,
      playerName,
