...
vi.mock('../components/ui/dialog', () => {
  return {
    DialogRoot: ({ open, onOpenChange, children }: { open: boolean, onOpenChange: (e: { open: boolean }) => void, children: ReactNode }) => (
      <div data-testid="dialog-root" data-open={open} onClick={() => onOpenChange({ open: false })}>
        {children}
      </div>
    ),
    DialogBackdrop: ({ style, ...props }: { style?: CSSProperties } & Record<string, unknown>) => <div data-testid="dialog-backdrop" style={style} {...props} />,
    DialogContent: ({ children, style, ...props }: { children: ReactNode, style?: CSSProperties } & Record<string, unknown>) => (
      <div data-testid="dialog-content" style={style} {...props}>
        {children}
      </div>
    ),
    DialogHeader: ({ children, style, css, ...rest }: { children: ReactNode, style?: CSSProperties, css?: CSSProperties } & Record<string, unknown>) => {
      // Merge style/css/spread props so toHaveStyle works in tests
      const merged = { ...(style || css || {}), ...rest } as CSSProperties;
      return <div data-testid="dialog-header" style={merged}>{children}</div>;
    },
    DialogBody: ({ children, style, css, ...props }: { children: ReactNode, style?: CSSProperties, css?: CSSProperties } & Record<string, unknown>) => <div data-testid="dialog-body" style={style || css} {...props}>{children}</div>,
    DialogCloseTrigger: ({ children, style, css, ...props }: { children: ReactNode, style?: CSSProperties, css?: CSSProperties } & Record<string, unknown>) => (
  ...
  it('applies custom content styles', () => {
    renderWithProvider(
      <AppModal
        ...
        contentProps={{ 'data-testid': 'custom-modal' } as Record<string, unknown>}
      />
    );
